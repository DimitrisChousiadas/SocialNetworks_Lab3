...
for crossoverProb = 0.7:0.1:0.9
    for mutationProb = 0.1:0.1:0.2
        for elitism = 1:1:3
            ...
            name = strcat('football_','c',int2str(crossoverProb/0.1),'m',int2str(mutationProb/0.1),'e',int2str(elitism),'.png');
            PlotGraph(football, communities);
            saveas(gcf,strcat('results/football/',name));

        end
    end
end

save('results/football.mat');